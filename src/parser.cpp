...
interpreter::RegID Parser::tokenTypeToReg(TokenType id) {
  if (id > vm_numbers::OPCODE_NUM - 1 &&
      id < vm_numbers::OPCODE_NUM + vm_numbers::REG_NUM - 1) {
    // then its int reg
...
void Parser::parseProgram(std::ifstream &file) {
  ...
  while (std::getline(file, line)) {
    std::pair<std::vector<Token>, LexerError> tokens_and_errors =
        lexer.makeTokensGreatAgain(line);
    ...
    for (int i = 0; i < len; ++i) {
      ...
      if (curr_token_type != TokenType::BAD_TOKEN &&
          curr_token_type < TokenType::ID_NUMBER - vm_numbers::VM_DIGIT_NUM) {
    ...
    if (op_number == 3) {
      program_.push_back(parseOpWith2Args(
          curr_set_to_instruction[curr_set_to_instruction.size() - 3],
          curr_set_to_instruction[curr_set_to_instruction.size() - 2],
          curr_set_to_instruction[curr_set_to_instruction.size() - 1]));
    } else if (op_number == 2) {
      program_.push_back(parseOpWith1Args(
          curr_set_to_instruction[curr_set_to_instruction.size() - 2],
          curr_set_to_instruction[curr_set_to_instruction.size() - 1]));
    } else if (op_number == 1) {
      program_.push_back(parseOpWithNoArgs(
          curr_set_to_instruction[curr_set_to_instruction.size() - 1]));
    } else {
