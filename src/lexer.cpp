...
#define OPLIST(code, mnemonic) {mnemonic, TokenType::OP_##code},
      ISA(OPLIST)
#undef OPLIST
  };

  return (operation_list.find(operation) == operation_list.end())
             ? (TokenType::BAD_TOKEN)
...
#define INT_REGLIST(code, mnemonic) {mnemonic, TokenType::REG_##code},
      INT_REGS(INT_REGLIST)
#undef INT_REGLIST
  };

  return (int_reg_list.find(reg) == int_reg_list.end())
             ? (TokenType::BAD_TOKEN)
             : (int_reg_list.at(reg));
}
...
#define FLOAT_REGLIST(code, mnemonic) {mnemonic, TokenType::REG_##code},
      FLOAT_REGS(FLOAT_REGLIST)
#undef FLOAT_REGLIST
  };

  return (float_reg_list.find(reg) == float_reg_list.end())
             ? (TokenType::BAD_TOKEN)
...
bool Lexer::lexString() {
  ...
  if (!isQuote(buffer_[position_])) {
    error_ = LexerError("no termination for string.", position_,
                        ErrorType::ERROR_UNEXPECTED_END_OF_LINE, buffer_);
...
void Lexer::lexTokens() {
  ...
  for (int i = 0; !endOfLine(); ++i) {
    ...
    if (isSpecialSymbol(buffer_[position_]) != TokenType::BAD_TOKEN) {
      ...
      while (!endOfLine() &&
             Lexer::isSpecialSymbol(buffer_[position_]) ==
                 TokenType::BAD_TOKEN &&
    ...
    tokens_.emplace_back(buffer_, bound_l, bound_r,
                         Lexer::getType(bound_l, bound_r));
