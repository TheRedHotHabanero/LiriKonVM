...
#define OPLIST(code, mnemonic) OP_##code,
  ISA(OPLIST)
#undef OPLIST

#define INT_REGLIST(code, mnemonic) REG_##code,
      INT_REGS(INT_REGLIST)
#undef INT_REGLIST

#define FLOAT_REGLIST(code, mnemonic) REG_##code,
...
struct Token {
  Token()
      : line_(""), bound_left_(0), bound_right_(0), token_type_(BAD_TOKEN) {}
  Token(std::string line, size_t bound_left, size_t bound_right,
        TokenType token_type)
      : line_(line), bound_left_(bound_left), bound_right_(bound_right),
...
struct LexerError {
  ...
  LexerError(std::string msg, size_t p, ErrorType err_type, std::string buff)
